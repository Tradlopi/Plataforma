...
def activate():
    try:
        ...
        if request.method == 'GET': 
            ...
            db = get_db()
            attempt = db.execute(
                #revisar si hay un cod q exista con las credenciales, si existe
            ...
            if attempt is not None:#se valida que sea diferente de NONE
                db.execute(#cual es el dato que estoy confirmando y cambiar e estado del campo state a confirmado
                    'UPDATE activationlink SET state = ? WHERE id = ?', 
                    (utils.U_CONFIRMED, attempt['id'])
                )
                db.execute(#insert inyo user ... guardar la info
...
def register():
    try:
        ...
        if request.method == 'POST':    
            ...
            )#guardar challenge, estado, username, password, salt, email 
            db.commit()
...
def confirm():
    try:
        ...
        if request.method == 'POST': 
            ...
            db = get_db()
            attempt = db.execute(#se debe hacer un select parecido al activation - validar que el numero del enlace sea valido
                'SELECT * FROM forgotlink  WHERE challenge = ? AND state = ?', (authid, utils.F_ACTIVE)
            ).fetchone()
            
            if attempt is not None:
                db.execute(#actualizar la tabla de fforgotlink - actualizar el estado como inactivo
                    'UPDATE forgotlink SET state = ? WHERE id = ?', (utils.F_INACTIVE, attempt['id'])
                )
                salt = hex(random.getrandbits(128))[2:]
                hashP = generate_password_hash(password + salt)   
                db.execute(#se guarda el nuevo password  - update
...
def change():
    try:
        ...
        if request.method == 'GET': 
            ...
            db = get_db()
            attempt = db.execute(
            'SELECT challenge, state FROM forgotlink WHERE challenge =? and state = ?' , (number, utils.F_ACTIVE)
             ).fetchone()
